...
export default function PropertyDetailClient({ property }: PropertyDetailClientProps) {
  ...
  return (
    <main className="min-h-screen bg-[#0D0501] text-[#E8DCBF] font-sans">
      ...
      <div className="print:hidden">
        {/* Breadcrumbs */}
        <div className="pt-[5.5rem] pb-3 px-6 lg:px-12 bg-[#180900] flex flex-wrap items-center justify-center gap-3 text-[8px] lg:text-[9px] font-bold tracking-[0.4em] text-[#E8DCBF]/40 uppercase border-b border-[#C6A75E]/10">
          <Link href="/properties" className="hover:text-[#E8DCBF] transition-colors">ALL PROPERTIES</Link>
          <span>/</span>
          <Link href={`/properties?search=${property.district?.name}`} className="hover:text-[#E8DCBF] transition-colors">
            {property.district?.name}
          </Link>
          <span>/</span>
          <span className="text-[#E8DCBF]">{property.title}</span>
        </div>
        ...
        {/* Property Header */}
        <motion.section
          className="py-12 lg:py-20 px-6 text-center max-w-4xl mx-auto border-b border-[#C6A75E]/10"
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.75, ease: [0.23, 1, 0.32, 1] }}
        >
          <div className="space-y-3 mb-8">
            <p className="text-[9px] font-bold tracking-[0.5em] text-[#C6A75E] uppercase">
              {property.propertyType?.[0] || "EXCLUSIVE LISTING"}
            </p>
            <h1 className="text-3xl lg:text-5xl font-serif tracking-tight text-[#E8DCBF] uppercase leading-tight">{property.title}</h1>
            <p className="text-[10px] tracking-[0.4em] text-[#E8DCBF]/40 uppercase font-bold flex items-center justify-center gap-2">
              <MapPin size={10} className="text-[#C6A75E]" />
              {property.buildingName && `${property.buildingName}, `}{property.district?.name}, {property.county}
            </p>
          </div>

          <div className="h-px w-8 bg-[#C6A75E] mx-auto mb-8" />

          <div className="mb-10">
            <p className="text-[9px] font-bold tracking-[0.5em] text-[#E8DCBF]/40 uppercase mb-3">OFFERED AT</p>
            {priceAmount ? (
              <CurrencyBadge
                amount={priceAmount}
                baseCurrency={priceCurrency}
                className="text-4xl lg:text-6xl font-serif text-[#E8DCBF] tracking-tighter justify-center"
              />
            ) : (
              <p className="text-4xl lg:text-5xl font-serif text-[#E8DCBF]">Price on Request</p>
            )}
          </div>
          ...
          <div className="flex flex-wrap justify-center items-center gap-10 lg:gap-20">
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-1">
                <p className="text-xl lg:text-2xl font-serif text-[#E8DCBF]">{stat.value}</p>
                <p className="text-[8px] font-bold tracking-[0.4em] text-[#E8DCBF]/40 uppercase">{stat.label}</p>
              </div>
            ))}
          </div>
        </motion.section>
        ...
        <section className="py-12 lg:py-20 px-6 lg:px-16 max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
          ...
          <div className="lg:col-span-8 space-y-16">
            ...
            {property.amenities && property.amenities.length > 0 && (
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="h-px w-10 bg-[#C6A75E]" />
                  <h2 className="text-[10px] font-bold tracking-[0.5em] uppercase text-[#C6A75E]">Features & Amenities</h2>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {property.amenities.map((amenity: string, i: number) => (
                    <div key={i} className="flex items-center gap-3 py-3 border-b border-[#C6A75E]/8">
                      <div className="w-1 h-1 bg-[#C6A75E] shrink-0" />
                      <span className="text-[10px] font-bold tracking-widest uppercase text-[#E8DCBF]/50">{amenity}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {property.verificationDocuments?.length > 0 && (
              <div className="bg-[#1E0D02] p-8 lg:p-12 space-y-8 border border-[#C6A75E]/10">
                <div>
            ...
            {/* Inquiry Form */}
            <div className="border border-[#C6A75E]/15 p-8 lg:p-12 space-y-8">
              <div>
                <div className="flex items-center gap-4 mb-2">
                  <div className="h-px w-10 bg-[#C6A75E]" />
                  <h2 className="text-[10px] font-bold tracking-[0.5em] uppercase text-[#C6A75E]">Make an Inquiry</h2>
                </div>
                <p className="text-sm text-[#E8DCBF]/50 font-light leading-relaxed">
              ...
              {inquirySent ? (
                <div className="flex flex-col items-center py-12 gap-4">
                  <div className="w-12 h-12 bg-[#C6A75E] flex items-center justify-center">
                    <Check size={20} className="text-white" />
                  </div>
                  <p className="text-[10px] font-bold tracking-[0.3em] uppercase">Inquiry Received</p>
                  <p className="text-sm text-[#E8DCBF]/40 text-center">Our team will be in touch within 24 hours.</p>
                </div>
              ) : (
                <form onSubmit={handleInquiry} className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-bold tracking-[0.3em] uppercase text-[#E8DCBF]/40">Full Name</label>
                      <input
                        type="text"
                        required
                        value={inquiryForm.name}
                        onChange={e => setInquiryForm(f => ({ ...f, name: e.target.value }))}
                        className="w-full border border-[#C6A75E]/15 bg-[#1E0D02] text-[#E8DCBF] placeholder:text-[#E8DCBF]/25 px-4 py-3 text-[11px] tracking-wide focus:outline-none focus:border-[#C6A75E] transition-colors bg-[#180900]"
                        placeholder="Your full name"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-bold tracking-[0.3em] uppercase text-[#E8DCBF]/40">Email Address</label>
                      <input
                        type="email"
                        required
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-bold tracking-[0.3em] uppercase text-[#E8DCBF]/40">Phone Number</label>
                    <input
                      type="tel"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-bold tracking-[0.3em] uppercase text-[#E8DCBF]/40">Inquiry Type</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-bold tracking-[0.3em] uppercase text-[#E8DCBF]/40">Message</label>
                    <textarea
                      rows={4}
                      value={inquiryForm.message}
                      onChange={e => setInquiryForm(f => ({ ...f, message: e.target.value }))}
                      className="w-full border border-[#C6A75E]/15 bg-[#1E0D02] text-[#E8DCBF] placeholder:text-[#E8DCBF]/25 px-4 py-3 text-[11px] tracking-wide focus:outline-none focus:border-[#C6A75E] transition-colors bg-[#180900] resize-none"
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-28 h-fit space-y-6">
            {/* Consultation Card */}
            <div className="border border-[#C6A75E]/15 p-8 space-y-8">
              <div>
                <p className="text-[10px] font-bold tracking-[0.5em] text-[#E8DCBF]/40 uppercase mb-2">Private Consultation</p>
                <p className="text-sm text-[#E8DCBF]/50 font-light leading-relaxed">
              ...
              <div className="pt-6 border-t border-[#C6A75E]/10 flex items-center gap-4">
                <div className="w-10 h-10 bg-[#0D0501] text-[#C6A75E] flex items-center justify-center font-serif text-lg">P</div>
                <div>
                  <p className="text-[10px] font-bold tracking-widest uppercase">Pavani Realty</p>
                  <p className="text-[8px] text-[#E8DCBF]/40 tracking-[0.2em] uppercase font-bold">Exclusive Listing Agent</p>
                </div>
              </div>
            </div>

            {/* Quick Actions */}
            <div className="flex justify-center gap-10 text-[#E8DCBF]/40 border border-[#C6A75E]/10 py-5">
              <button onClick={handleShare} className="flex flex-col items-center gap-2 hover:text-[#E8DCBF] transition-colors">
                <Share2 size={18} /><span className="text-[8px] font-bold uppercase tracking-widest">Share</span>
              </button>
              <button onClick={() => window.print()} className="flex flex-col items-center gap-2 hover:text-[#E8DCBF] transition-colors">
                <Printer size={18} /><span className="text-[8px] font-bold uppercase tracking-widest">Print</span>
              </button>
              <button onClick={toggleLike} className={cn("flex flex-col items-center gap-2 transition-colors", isLiked ? "text-[#C6A75E]" : "hover:text-[#E8DCBF]")}>
        ...
        {property.similarProperties?.length > 0 && (
          <section className="py-24 bg-[#180900] px-6 lg:px-16 border-t border-[#C6A75E]/10">
            <div className="text-center space-y-3 mb-16">
              <p className="text-[9px] font-bold tracking-[0.5em] text-[#C6A75E] uppercase">Curated Portfolio</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[1700px] mx-auto">
              {property.similarProperties.map((p: any) => (
                <Link key={p._id} href={`/properties/${p.slug}`} className="group bg-[#180900] border border-[#C6A75E]/10 block">
                  ...
                  <div className="p-6 space-y-4">
                    <div>
                      <p className="text-xl font-serif text-[#E8DCBF] mb-1">
                        {globalFormatPrice(typeof p.price === 'object' ? p.price.amount : p.price, typeof p.price === 'object' ? p.price.currency : "USD")}
                      </p>
                      <h3 className="text-[10px] font-bold tracking-widest text-[#E8DCBF]/40 uppercase line-clamp-1">{p.title}</h3>
                    </div>
                    <div className="pt-4 border-t border-[#C6A75E]/8 flex items-center justify-between text-[9px] font-bold tracking-widest text-[#E8DCBF]/40 uppercase">
